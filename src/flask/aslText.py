...
def text_mode(cam):
    ...
    # dictToSend = {'gesture':text}
    # res = requests.post('http://localhost:5000/test', json=dictToSend)
...
def get_asl_word():
    ...
    return text_mode(cam)


@app.route('/asltotext', methods=['POST', 'GET'])
@cross_origin(support_credentials=True)
def asl_to_text_controller():
    response = get_asl_word()
    response_data = {"aslToTextOutput": response}
